...
trait WirelessTermBase extends PowerItem{
  ...
  override def addInformation(itemStack: ItemStack, player: EntityPlayer, list: util.List[_], par4: Boolean) {
    ...
    list2.add(StatCollector.translateToLocal("gui.appliedenergistics2.StoredEnergy") + ": %,d".format(Math.ceil(aeCurrentPower).asInstanceOf[Int]) + " AE - " + Math.floor(aeCurrentPower / this.MAX_POWER * 1e4) / 1e2 + "%")
